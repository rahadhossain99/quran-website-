...
export const ReaderView = () => {
  const {
    ...
    arabicFontSize,
    bengaliFontSize,
    initialTargetAyahIndex,
    setInitialTargetAyahIndex,
    setArabicFontSize,
    setBengaliFontSize,
  } = useAppStore();
  ...
  const convertBantoEng = (str: string) => {
    ...
    return str.replace(
      /[০-৯]/g,
      (digit) => banglaDigits[digit as keyof typeof banglaDigits],
    );
  };
  ...
  useEffect(() => {
    if (currentViewSurah) {
      ...
      fetchSurahDetails(currentViewSurah, qari)
        .then((res) => {
          setData(res);
          setLoading(false);
        })
  ...
  useEffect(() => {
    if (
      data &&
      currentViewSurah === data.number &&
      initialTargetAyahIndex !== null
    ) {
  ...
  const isCurrentSurahPlaying =
    playingSurah?.number === data?.number && isPlaying;

  const filteredAyahs =
    data?.ayahs.filter((ayah) => {
  ...
  useEffect(() => {
    if (
      ...
      playingAyahIndex >= 0 &&
      ayahRefs.current[playingAyahIndex]
    ) {
      if (autoScrollAyah) {
          ...
          <>
            ...
            >
              ...
              <div className="flex justify-between items-start mb-6 relative z-10">
                ...
                >
                  <Bookmark
                    className={`w-5 h-5 ${favorites.includes(data.number) ? "fill-white text-white" : "text-white"}`}
                  />
                </button>
              </div>

              <h2
                className="text-5xl font-bold font-arabic text-[var(--accent)] mb-4 drop-shadow-lg relative z-10 leading-normal"
                dir="rtl"
              >
                {data.name}
              </h2>
              <h3 className="text-3xl font-bold text-white font-sans relative z-10 tracking-tight">
                {data.englishName}
              </h3>
              <p className="text-sm font-bold text-[var(--primary-soft)] mb-8 relative z-10">
                ...
                <span>
                  {isCurrentSurahPlaying
                    ...
                    : qari === "special.bangla_translation"
                      ? "আরবি ও বাংলা অনুবাদসহ তিলাওয়াত শুনুন"
                      : "সম্পূর্ণ তেলাওয়াত শুরু করুন"}
                </span>
              </button>
            </motion.div>
            ...
            {data.number !== 9 && data.number !== 1 && (
              <div className="text-center py-6 mb-8 bg-[var(--bg-surface)] shadow-sm relative overflow-hidden rounded-[2rem] border border-[var(--border)]">
                <p className="text-arabic font-arabic text-[var(--primary)] drop-shadow-sm text-center">
            ...
            <div className="mb-6 sticky top-2 z-30 grid grid-cols-1 md:grid-cols-12 gap-3 pb-2">
              ...
              <div className="md:col-span-3 bg-[var(--bg-surface)] p-2 rounded-[1.5rem] shadow-sm border border-[var(--border)] flex items-center justify-between px-3">
                <span className="text-xs font-bold text-[var(--text-muted)] whitespace-nowrap font-bengali">
                  যান:
                </span>
                <div className="flex items-center space-x-1.5 flex-1 justify-end">
                  ...
                  />
                  <span className="text-xs font-bold text-[var(--text-muted)] font-bengali">
                    নম্বর
                  </span>
                </div>
              </div>
                  ...
                  <div className="bg-[var(--bg-surface)] p-5 md:p-6 rounded-[2rem] border border-[var(--border)] shadow-sm grid grid-cols-1 md:grid-cols-2 gap-6 pb-2">
                    ...
                    <div className="space-y-2.5">
                      <div className="flex justify-between items-center px-1">
                        <span className="text-xs font-extrabold text-[var(--text-main)] font-bengali">
                          আরবি হরফের আকার:
                        </span>
                        <span className="text-xs font-black text-[var(--primary)] font-sans bg-[var(--bg-main)] px-2.5 py-1 rounded-lg border border-[var(--border)]">
                          {arabicFontSize}px
                        </span>
                      </div>
                      <div className="flex items-center space-x-4">
                        ...
                        <button
                          onClick={() =>
                            setArabicFontSize(Math.min(60, arabicFontSize + 2))
                          }
                          className="w-10 h-10 rounded-xl bg-[var(--bg-main)] text-[var(--text-main)] font-bold text-lg border border-[var(--border)] hover:bg-[var(--bg-surface)] hover:text-[var(--primary)] hover:border-[var(--primary)] transition-colors active:scale-90"
                        >
                          +
                    ...
                    <div className="space-y-2.5">
                      ...
                      <div className="flex items-center space-x-4">
                        ...
                        <input
                          ...
                          onChange={(e) =>
                            setBengaliFontSize(Number(e.target.value))
                          }
                          className="flex-1 accent-[var(--primary)] bg-[var(--border)] h-1 rounded-lg appearance-none cursor-pointer"
                        />
                        <button
                  ...
                  >
                    ...
                    <div className="flex items-center justify-between mb-6 pb-4 border-b border-[var(--border)] opacity-80">
                      <div className="flex items-center gap-2">
                        ...
                        {isActive && isPlaying && qari === "special.bangla_translation" && (
                          <span className={`text-[10px] font-bold px-2.5 py-1 rounded-full transition-all ${
                            ...
                            {audioPhase === "bangla" ? "🔊 বাংলা অনুবাদ বাজছে..." : "🔊 আরবি তিলাওয়াত বাজছে..."}
                          </span>
                        )}
                      </div>
                    ...
                    <div className={`text-right mb-6 p-6 rounded-3xl border w-full overflow-x-auto min-h-max transition-all duration-300 ${
                      isActive && isPlaying && qari === "special.bangla_translation" && audioPhase === "arabic"
                        ? "bg-[var(--bg-main)] border-[var(--primary)] ring-2 ring-[var(--primary)]/30 shadow-md"
                        : "bg-[var(--bg-main)] border-[var(--border)]"
                    }`}>
                      <p
                        className="font-arabic text-[var(--text-main)] drop-shadow-sm min-h-full"
                        dir="rtl"
                        style={{
                      ...
                      >
                        {ayah.arabicText}{" "}
                        <span
                          className="text-[var(--primary)] opacity-70 mx-2 font-sans inline-flex items-center"
                    ...
                    <div className={`space-y-4 px-3 py-3 rounded-2xl w-full min-h-max transition-all duration-300 ${
                      isActive && isPlaying && qari === "special.bangla_translation" && audioPhase === "bangla"
                      ...
                      <div className="bg-[var(--bg-main)] rounded-xl p-4 border border-[var(--border)] mt-2">
                        <h4 className="text-[10px] font-bold text-[var(--primary)] uppercase tracking-wider mb-2 font-sans opacity-80">
                          উচ্চারণ
                        </h4>
                        <p className="font-semibold text-[var(--text-muted)] font-sans text-sm">
